...
function baw_theme_setup() {
	...
	add_image_size('blog_thumb', 249, 160 ,true );
	add_image_size('case_thumb', 260, 160 ,true );
	add_image_size('case_ba', 390, 220 ,true );
}
...
add_filter( 'mwform_custom_mail_tag_mw-wp-form-918', 'send_date_time', 10, 3 );
add_filter( 'mwform_custom_mail_tag_mw-wp-form-2666', 'send_date_time', 10, 3 );

//MW WP FORM パラメータ
function my_mwform_value($value, $name) {
	if ($name === 'userid' && !empty($_GET['salesid']) && !is_array($_GET['salesid'])) {
		return $_GET['salesid'];
	}
	return $value;
}
add_filter('mwform_value_mw-wp-form-98', 'my_mwform_value', 10, 4);
add_filter('mwform_value_mw-wp-form-374', 'my_mwform_value', 10, 4);
add_filter('mwform_value_mw-wp-form-449', 'my_mwform_value', 10, 4);
add_filter('mwform_value_mw-wp-form-525', 'my_mwform_value', 10, 4);
add_filter('mwform_value_mw-wp-form-2666', 'my_mwform_value', 10, 4);


function my_required_times1( $validation, $data ) {
  $method = $data['radio4'];
  if( isset( $method ) && $method === '利用したことがある' ) {
    $validation->set_rule( 'times', 'noEmpty', array( 'message' => '利用回数を入力ください。') );
  }
  return $Validation;
}

add_filter( 'mwform_validation_mw-wp-form-98', 'my_required_times1', 10, 2 );
add_filter( 'mwform_validation_mw-wp-form-374', 'my_required_times1', 10, 2 );

function my_required_times2( $validation, $data ) {
  $method = $data['radio8'];
  if( isset( $method ) && $method === '利用したことがある' ) {
    $validation->set_rule( 'times', 'noEmpty', array( 'message' => '利用回数を入力ください。') );
  }
...
add_filter( 'mwform_validation_mw-wp-form-449', 'my_required_times2', 10, 2 );

function my_required_times3( $validation, $data ) {
  $method = $data['radio7'];
  if( isset( $method ) && $method === '利用したことがある' ) {
    $validation->set_rule( 'times', 'noEmpty', array( 'message' => '利用回数を入力ください。') );
  }
...
add_filter( 'mwform_validation_mw-wp-form-2666', 'my_required_times3', 10, 2 );
